...
|---|---|---|---|
| **Core plugins** (`bundled: true`) | `slopsmith/slopsmith/plugins/` | Docker image + desktop | Yes — tied to core release |
| **Org plugins** | `slopsmith/slopsmith-plugin-*` repos | Desktop only | No — independent cadence |
...
Core plugins have `"bundled": true` in their `plugin.json`. They live directly in the `slopsmith/slopsmith` repository under `plugins/` and are the source of truth for what ships in the Docker image.
...
Org plugins live in their own repos under the `slopsmith` GitHub org (`slopsmith/slopsmith-plugin-*`). The desktop build clones them at build time via `build-common.sh`.
...
```yaml
# .github/workflows/ci.yml in any slopsmith/slopsmith-plugin-* repo
jobs:
  plugin-lint:
