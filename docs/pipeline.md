...
Org plugins (`slopsmith/slopsmith-plugin-*`) are cloned by the desktop build at build time. For release builds, they are pinned to specific commits recorded in `plugin-lock.json` on the active `release/*` branch. Nightlies always use tip.
...
```json
{
  "slopsmith/slopsmith-plugin-notedetect": "abc1234",
  "slopsmith/slopsmith-plugin-piano": "def5678",
